...
class Train(trainunits.Units):
    ...
    def discriminator_trainstep(self, images, b_size):
        ...
        return real_loss, fake_loss, r2_reg, gp
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            for i, (images, _) in enumerate(self.dataloader):
                ...
                real_loss, fake_loss, r2_reg, gp = self.discriminator_trainstep(images, b_size)
                gen_imgs, g_loss = self.generator_trainstep(b_size)

                self.show_images(gen_imgs, b_size)
                # self.show_images(torch.cat([gen_imgs, images], 0), b_size)
                ...
                proc_bar.set_postfix(
                    {"epoch": f"{epoch}", "Loss_G": f"{g_loss.item():.4f}", "real_loss": f"{real_loss.item():.4f}",
                     "fake_loss": f"{fake_loss.item():.4f}", "r2_reg": f"{r2_reg.item():.4f}",
                     "gp": f"{gp.item():.4f}"})
                self.scheduler_G.step()
                self.scheduler_D.step()
                proc_bar.update(1)
            self.save_ckpt('WGANGPSNR2', epoch + 1, 0)
            proc_bar.reset()
        proc_bar.close()
