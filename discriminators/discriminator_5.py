...
class ResnetInit(nn.Module):
    def __init__(self, in_channel, out_channel, stride):
        ...
        self.residual_bn_LeakyReLU = nn.Sequential(
            nn.BatchNorm2d(out_channel),
            nn.LeakyReLU(0.2, inplace=True),
        )

        self.transient_bn_LeakyReLU = nn.Sequential(
            nn.BatchNorm2d(out_channel),
            nn.LeakyReLU(0.2, inplace=True),
        )
    ...
    def forward(self, x):
        ...
        x_residual = self.residual_bn_LeakyReLU(residual_r_r + transient_t_r + residual_shortcut)
        x_transient = self.transient_bn_LeakyReLU(residual_r_t + transient_t_t)
