...
class SKStyleConv(nn.Module):
    def __init__(self, d_latent: int, in_planes: int, out_planes: int, m: int, image_size: int, use_noise: bool):
        super(SKStyleConv, self).__init__()
        assert (m > 0)
        self.M = m
        for i in range(m):
            conv = StyleConv(d_latent, in_planes, out_planes, kernel_size=3 + i * 2, use_noise=use_noise)
            self.__setattr__('conv_%d' % i, conv)
            nonlinear = nn.PReLU(out_planes)
            self.__setattr__('nonlinear_%d' % i, nonlinear)
...
class StyleBlock(nn.Module):
    def __init__(self, d_latent: int, last_planes: int, in_planes: int, out_planes: int, dense_depth: int,
        ...
        self.m = m
        if m == 1:
            self.conv2 = StyleConv(d_latent, in_planes, in_planes, kernel_size, True)
            self.activation2 = nn.PReLU(in_planes)
        else:
            self.skconv = SKStyleConv(d_latent, in_planes, in_planes, m, image_size, True)
        self.conv3 = StyleConv(d_latent, in_planes, out_planes + dense_depth, kernel_size=3)
...
class ResnetInit(nn.Module):
    def __init__(self, d_latent: int, last_planes: int, in_planes: int, out_planes: int, dense_depth: int,
        ...
        self.residual = StyleBlock(d_latent, last_planes, in_planes, out_planes, dense_depth, kernel_size, m,
                                   image_size)
        self.transient = StyleBlock(d_latent, last_planes, in_planes, out_planes, 0, kernel_size, m, image_size)
        self.residual_across = StyleBlock(d_latent, last_planes, in_planes, out_planes, 0, kernel_size, m, image_size)
    ...
    def forward(self, x: Tuple[torch.Tensor, torch.Tensor], w: torch.Tensor):
        ...
        feas_residual = self.activation_residual(residual_r_r + transient_t_r)
        feas_transient = self.activation_transient(residual_r_t + transient_t_t)

        return feas_residual, feas_transient
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, d_latent: int, last_planes: int, in_planes: int, out_planes: int, dense_depth: int, root: bool,
        ...
        if is_unify:
            ...
            self.rir_3 = ResnetInit(d_latent, last_planes - out_planes, in_planes, out_planes, dense_depth,
                                    3, m, image_size)
        ...
        if image_size > 4:
            ...
            self.se_attention_residual = SEBlock_fc(out_planes)
    ...
    def forward(self, x: torch.Tensor, w: torch.Tensor):
        ...
        feas_residual = x[:, :d, :, :] + x_residual_3[:, :d, :, :] * self.se_attention_residual(x_residual_3[:, :d, :, :])
...
class ToRGB(nn.Module):
    def __init__(self, planes: int, m: int, image_size: int):
        ...
        self.conv = EqualizedConv2d(planes, 3, kernel_size=5)
