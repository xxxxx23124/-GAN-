...
# Pixnorm from PROGRESSIVE GROWING OF GANS FOR IMPROVED QUALITY, STABILITY, AND VARIATION's PIXELWISE FEATURE VECTOR NORMALIZATION IN GENERATOR
class Pixnorm(torch.nn.Module):
    ...
    def forward(self, x):
        normalizer = torch.sqrt(1e-8 + torch.sum(x ** 2.0, dim=1, keepdim=True) / self.in_planes)
        return x.div(normalizer)


'''
class EqualizedConv2d(torch.nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, downscale=False, groups=1,
                 padding_mode='reflect'):
        super().__init__()
        self.weights = torch.nn.Parameter(
            torch.nn.init.normal_(torch.empty(out_planes, in_planes//groups, kernel_size, kernel_size)))
        self.stride = stride
        self.padding = padding
        self.downscale = downscale
        self.groups = groups
        self.padding_mode = padding_mode
        self.bias = torch.nn.Parameter(torch.nn.init.normal_(torch.empty(out_planes)))
        fan_in = kernel_size * kernel_size * in_planes
    ...
    def forward(self, x):
        if self.downscale:
            ...
            weight = (weight[:, :, :-1, :-1] + weight[:, :, :-1, 1:] +
                      weight[:, :, 1:, :-1] + weight[:, :, 1:, 1:]) / 4.0
        else:
            weight = self.weights
        x = torch.nn.functional.pad(input=x, pad=(self.padding, self.padding, self.padding, self.padding),
                                    mode=self.padding_mode)
        return torch.nn.functional.conv2d(input=x, weight=weight * self.scale, bias=self.bias, stride=self.stride,
                                          groups=self.groups)
'''


# EqualizedConv2d block from PROGRESSIVE GROWING OF GANS FOR IMPROVED QUALITY, STABILITY, AND VARIATION's 4.1 EQUALIZED LEARNING RATE
...
class EqualizedConvTranspose2D(torch.nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, output_padding=0, upscale=False):
        super().__init__()
        self.weights = torch.nn.Parameter(
            torch.nn.init.normal_(torch.empty(in_planes, out_planes, kernel_size, kernel_size)))
        self.stride = stride
        self.padding = padding
    ...
    def forward(self, x):
        if self.upscale:
            ...
            weight = self.weights
        return torch.nn.functional.conv_transpose2d(input=x, weight=weight * self.scale, bias=self.bias,
...
# SelfAttention block from Self-Attention Generative Adversarial Networks
class SelfAttention(nn.Module):
    def __init__(self, in_planes, embedding_channels):
        super(SelfAttention, self).__init__()
        self.key = EqualizedConv2d(in_planes=in_planes, out_planes=embedding_channels, kernel_size=1)
        self.query = EqualizedConv2d(in_planes=in_planes, out_planes=embedding_channels, kernel_size=1)
        self.value = EqualizedConv2d(in_planes=in_planes, out_planes=embedding_channels, kernel_size=1)
        self.self_att = EqualizedConv2d(in_planes=embedding_channels, out_planes=in_planes, kernel_size=1)
...
# SEBottleneck form MobileNetV2: Inverted Residuals and Linear Bottlenecks and Squeeze-and-Excitation Networks's SE-ResNet Module
class SEBottleneck(nn.Module):
    ...
    def get_conv_bottleneck(self, last_planes, in_planes, out_planes, dense_depth, kernel_size):
        return nn.Sequential(
            EqualizedConv2d(in_planes=last_planes, out_planes=in_planes, kernel_size=1, stride=1, padding=0),
            Pixnorm(in_planes),
            nn.PReLU(),
            EqualizedConv2d(in_planes=in_planes, out_planes=in_planes, kernel_size=kernel_size, stride=1,
                            padding=(kernel_size - 1) // 2, groups=in_planes, padding_mode='reflect'),
            Pixnorm(in_planes),
            nn.PReLU(),
            EqualizedConv2d(in_planes=in_planes, out_planes=out_planes + dense_depth, kernel_size=1, stride=1,
                            padding=0),
            Pixnorm(out_planes + dense_depth),
        )

    def get_se(self, in_planes, out_planes, dense_depth, feature_size):
        return nn.Sequential(
            nn.AvgPool2d(kernel_size=feature_size, padding=0),
            EqualizedConv2d(in_planes=out_planes + dense_depth, out_planes=in_planes, kernel_size=1),
            nn.PReLU(),
            EqualizedConv2d(in_planes=in_planes, out_planes=out_planes + dense_depth, kernel_size=1),
            nn.Sigmoid()
        )
...
# ResnetInit from RESNET IN RESNET: GENERALIZING RESIDUAL ARCHITECTURES
class ResnetInit(nn.Module):
    def __init__(self, last_planes, in_planes, out_planes, dense_depth, kernel_size, feature_size):
        super(ResnetInit, self).__init__()
        self.residual = SEBottleneck(last_planes, in_planes,
                                     out_planes, dense_depth, kernel_size, feature_size)
        self.transient = SEBottleneck(last_planes, in_planes,
                                      out_planes, 0, kernel_size, feature_size)
        self.residual_across = SEBottleneck(last_planes, in_planes,
                                            out_planes, 0, kernel_size, feature_size)
        self.transient_across = SEBottleneck(last_planes, in_planes,
                                             out_planes, dense_depth, kernel_size, feature_size)
...
# BasicBlock is the basic blook of the Tree block and from Dual Path Networks's DPN
class BasicBlock(nn.Module):
    ...
    def __init__(self, last_planes, in_planes, out_planes, dense_depth, root, feature_size, is_unify):
        ...
        # is_unify is using 1x1 conv to let the in channel fit the last concatenating function
        if is_unify:
            self.unify = nn.Sequential(
                EqualizedConv2d(in_planes=last_planes, out_planes=2 * out_planes + dense_depth, kernel_size=1),
                Pixnorm(2 * out_planes + dense_depth)
            )
        ...
        # root is using 1x1 conv to let the out channel of Tree be
        if root:
            self.shortcut = nn.Sequential(
                EqualizedConv2d(in_planes=last_planes, out_planes=2 * out_planes + dense_depth, kernel_size=1),
                Pixnorm(2 * out_planes + dense_depth)
...
# Tree is from Deep Layer Aggregation's Hierarchical deep aggregation
...
class Generator(nn.Module):
    ...
    def get_upsample(self, planes, out_planes, kernel_size, stride, padding):
        return nn.Sequential(
            EqualizedConvTranspose2D(in_planes=planes, out_planes=out_planes, kernel_size=kernel_size, stride=stride,
                                     padding=padding),
            Pixnorm(out_planes),
            nn.PReLU(),
        )
    ...
    def __init__(self, z_dim):
        ...
        t2 = Tree(planes * 4, in_planes=planes, out_planes=planes, dense_depth=planes // 8, level=1,
                  block_num=1,
                  feature_size=16)
        ...
        t4 = Tree(planes, in_planes=planes // 4, out_planes=planes // 4, dense_depth=planes // 32, level=1,
                  block_num=1,
                  feature_size=64)
        ...
        self.LastConv = EqualizedConv2d(in_planes=last_planes, out_planes=3, kernel_size=9, stride=1, padding=4,
                                        padding_mode='reflect')
